...
const GameSandbox: FC = () => {
  const [gameState, setGameState] = useState<'loading' | 'tutorial' | 'menu' | 'playing' | 'gameover' | 'finality'>('loading');
  ...
  const [flames, setFlames] = useState<Array<{ id: number; x: number; y: number; vx: number; vy: number; life: number; color: string; size: number }>>([]);

  // NEW: Network Congestion mechanic
  const [networkCongested, setNetworkCongested] = useState(false);
  const [congestionDropsLeft, setCongestionDropsLeft] = useState(0);
  const [baseSpeed, setBaseSpeed] = useState(1.5);
  const [baseDifficulty, setBaseDifficulty] = useState(0.8);

  // NEW: Validator rotation
  const [currentValidator, setCurrentValidator] = useState(0);
  const validators = ['Jump Crypto', 'Firedancer', 'Solana Labs', 'Helius', 'Jito Labs', 'Triton', 'Everstake'];
  ...
  // Rotate validator every 3 seconds
  useEffect(() => {
    if (gameState === 'playing') {
      const interval = setInterval(() => {
        setCurrentValidator((prev) => (prev + 1) % validators.length);
      }, 3000);
      return () => clearInterval(interval);
    }
  }, [gameState, validators.length]);
  ...
  useEffect(() => {
    if (blocks.length > 1 && blocks.length % 5 === 0) {
      ...
      setBaseSpeed(Math.min(newSpeed, 4.5));
      setBaseDifficulty(Math.min(newDifficulty, 2.5));
  ...
  // NEW: Network Congestion Trigger (every 15-20 blocks)
  useEffect(() => {
    if (blocks.length > 1 && blocks.length % 17 === 0 && !networkCongested) {
      // Trigger congestion
      setNetworkCongested(true);
      setCongestionDropsLeft(4); // Lasts for 4 drops

      const congestionMsg = solanaMessages.congestion[Math.floor(Math.random() * solanaMessages.congestion.length)];
      setFeedback({ text: congestionMsg, subtext: 'Timing will be harder!', color: '#FFA500' });
      vibrate([100, 50, 100, 50, 100]);
      playWarningSound();

      setTimeout(() => setFeedback(null), 2000);
    }
  }, [blocks.length, networkCongested]);

  // NEW: Handle congestion countdown
  useEffect(() => {
    if (networkCongested && congestionDropsLeft === 0) {
      setNetworkCongested(false);
      setSpeed(baseSpeed);
      setDifficulty(baseDifficulty);
    }
  }, [congestionDropsLeft, networkCongested, baseSpeed, baseDifficulty]);

  // Flame particle physics animation
  useEffect(() => {
    if (gameState !== 'playing') {
      setFlames([]);
      return;
  ...
  // NEW: Warning sound for congestion
  const playWarningSound = () => {
    playSound(400, 0.1, 'square', 0.2);
    setTimeout(() => playSound(300, 0.1, 'square', 0.2), 100);
    setTimeout(() => playSound(400, 0.1, 'square', 0.2), 200);
  };

  const spawnFlames = (x: number, y: number) => {
    const flameColors = ['#FF4500', '#FF6347', '#FFA500', '#FFD700', '#FF1493', '#FF69B4'];
    const newFlames = Array.from({ length: 30 }, (_, i) => {
      ...
      return {
        ...
        vy: Math.sin(angle) * speed - Math.random() * 8 - 2,
        life: 1.0,
    ...
    if (gameState === 'playing' && currentBlock && !isDropping) {
      ...
      const animate = (currentTime: number) => {
        ...
        setCurrentBlock((prev) => {
          ...
          // NEW: Add jitter during congestion
          let actualSpeed = speed;
          if (networkCongested) {
            const jitter = (Math.random() - 0.5) * 0.4;
            actualSpeed = speed + jitter;
          }

          const moveAmount = prev.direction * actualSpeed * difficulty * Math.min(deltaTime, 2);
  ...
  }, [gameState, isDropping, speed, difficulty, networkCongested]);
  ...
  const shareScore = () => {
    // NEW: Better share text
    const text = `I just confirmed ${totalBlocks} Solana blocks before the network failed ⚡\n\n🧱 Solana Stacker\nPerfect stacks = instant finality\nMiss = network error\n\n#ScrollyGameJam #Solana @Scrollyfeed @SuperteamUK`;
  ...
  const dropBlock = () => {
    ...
    // NEW: Decrement congestion counter
    if (networkCongested && congestionDropsLeft > 0) {
      setCongestionDropsLeft((prev) => prev - 1);
    }
    ...
    if (overlapWidth <= 0) {
      ...
      // NEW: Show finality screen first
      setTimeout(() => {
        setGameState('finality');
        vibrate([200, 100, 200]);

        // Then game over after 2 seconds
        setTimeout(() => {
    ...
    // NEW: Adjust tolerance during congestion
    let perfectTolerance = 10;
    let goodTolerance = 22;

    if (networkCongested) {
      perfectTolerance = 6; // Harder!
      goodTolerance = 15;
    }
    ...
    if (isPerfect) {
      ...
      spawnFlames(currentX + currentWidth / 2, blocks.length * 40 + 20);
      blockType = 'perfect';
    } else if (isGood) {
      points = 2 * multiplier;
  ...
  if (gameState === 'tutorial') {
    const tutorialSteps = [
      ...
      },
      {
        ...
        description: 'The higher you stack, the FASTER it gets! Every 5 blocks = speed boost. Watch out for network congestion events!',
        highlight: 'Network congestion makes timing harder!',
          ...
          <div className="text-center space-y-6 w-full max-w-md my-4">
            ...
            <div className="bg-black/50 backdrop-blur-md rounded-3xl p-6 space-y-3 text-left text-sm border-2 border-purple-500/30 shadow-xl">
              ...
              <p className="flex items-center gap-3">
                ...
                <span className="font-medium">Network congestion = harder timing!</span>
              </p>
              <p className="flex items-center gap-3">
                <span className="text-2xl">💀</span>
  ...
  // NEW: Finality Screen
  if (gameState === 'finality') {
    return (
      <div className="flex flex-col items-center justify-center h-screen bg-gradient-to-br from-gray-950 via-purple-950 to-black text-white p-6">
        <div className="text-center space-y-6 animate-pulse">
          <div className="text-9xl mb-6">⛓️</div>
          <h1 className="text-6xl font-black bg-gradient-to-r from-purple-400 to-blue-400 bg-clip-text text-transparent leading-tight">
            FINAL STATE<br />COMMITTED
          </h1>
          <div className="bg-black/60 backdrop-blur-xl rounded-3xl p-8 border-2 border-purple-500/50 space-y-4 mt-8">
            <div>
              <p className="text-sm text-gray-400 font-bold">BLOCK HEIGHT</p>
              <p className="text-6xl font-black text-purple-400">{totalBlocks}</p>
            </div>
            <div className="h-px bg-gradient-to-r from-transparent via-purple-500 to-transparent" />
            <div>
              <p className="text-sm text-gray-400 font-bold">SOL EARNED</p>
              <p className="text-6xl font-black text-yellow-400">{score}</p>
            </div>
          </div>
          <p className="text-gray-500 text-sm font-semibold mt-6">Broadcasting to network...</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className={`flex flex-col h-screen bg-gradient-to-br from-slate-950 via-indigo-950 to-purple-950 text-white relative overflow-hidden ${shakeScreen ? 'animate-shake' : ''} ${networkCongested ? 'bg-red-overlay' : ''}`}>
      <style>{`
        @keyframes shake {
        ...
        .validator-text {
          animation: validatorSlide 0.5s ease-out;
        }
      `}</style>

      {/* NEW: Network Congestion Overlay */}
      {networkCongested && (
        <div className="absolute top-0 left-0 right-0 bg-gradient-to-b from-red-600/40 to-transparent py-3 z-30 pointer-events-none border-b-2 border-red-500/50">
          <p className="text-center text-2xl font-black text-red-200 animate-pulse drop-shadow-2xl">
            ⚠️ NETWORK CONGESTED
          </p>
          <p className="text-center text-sm text-red-300 font-bold mt-1">
            Timing tolerance reduced! ({congestionDropsLeft} drops remaining)
          </p>
        </div>
      )}
        ...
        <div className="relative mx-auto" style={{ width: '300px', height: '380px' }}>
          ...
          >
            ...
            {currentBlock && (
              <div
                ...
                style={{
                  ...
                  height: '38px',
                  backgroundColor: networkCongested ? '#FF4500' : '#F59E0B',
                  border: `4px solid ${networkCongested ? '#FF6347' : '#FDE047'}`,
                  borderRadius: '8px',
                  boxShadow: networkCongested
                    ? '0 0 50px rgba(255, 69, 0, 1), 0 0 100px rgba(255, 69, 0, 0.6)'
                    : '0 0 50px rgba(245, 158, 11, 1), 0 0 100px rgba(245, 158, 11, 0.6), 0 0 150px rgba(245, 158, 11, 0.3)',
                  zIndex: 10,
                  transform: 'translateZ(0)'
                }}
              >
            ...
            <div className="absolute -bottom-4 left-0 w-full">
              ...
              {/* NEW: Rotating Validator Name */}
              <div className="text-center mt-3">
                <p key={currentValidator} className="text-xs text-purple-300 font-bold validator-text">
                  Validator: {validators[currentValidator]}
                </p>
              </div>
            </div>
          </div>
      ...
      <div className="p-4 pb-safe flex-shrink-0 z-20">
        <button
          onClick={dropBlock}
          disabled={isDropping}
          className={`w-full bg-gradient-to-r from-green-600 via-emerald-600 to-green-600 hover:from-green-700 hover:via-emerald-700 hover:to-green-700 text-white font-black py-6 rounded-3xl text-3xl shadow-2xl active:scale-95 transition-all duration-150 border-4 border-white/40 relative overflow-hidden ${isDropping ? 'opacity-60 cursor-not-allowed' : ''} ${networkCongested ? 'border-red-500 from-red-600 via-orange-600 to-red-600' : ''}`}
        >
          <span className="relative z-10 drop-shadow-lg">
            {networkCongested ? '⚠️ DROP BLOCK' : '📦 DROP BLOCK'}
          </span>
          <div className="absolute inset-0 bg-gradient-to-r from-transparent via-white/30 to-transparent animate-pulse" />
        </button>
        <p className="text-center text-xs text-gray-400 mt-2 font-bold">
          {networkCongested ? 'Network unstable - be precise!' : 'Tap to confirm Solana transaction'}
        </p>
      </div>
    </div>
  );
